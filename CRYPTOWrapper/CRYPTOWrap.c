...
void ASN1_UTCTIME2TM(const ASN1_TIME *asnt, struct tm *tm)
/*Procede de OpenSSL time_t ASN1_UTCTIME_get(const ASN1_UTCTIME *s)*/
{
  int offset;
  ...
  tm->tm_year=g2(asnt->data);
  if(tm->tm_year<50)
    tm->tm_year+=100;
  tm->tm_mon=g2(asnt->data+2)-1;
  tm->tm_mday=g2(asnt->data+4);
  tm->tm_hour=g2(asnt->data+6);
  tm->tm_min=g2(asnt->data+8);
  tm->tm_sec=g2(asnt->data+10);
  if(asnt->data[12]=='Z')
    offset=0;
  else {
    offset=g2(asnt->data+13)*60+g2(asnt->data+15);
    if(asnt->data[12]=='-')
    offset=-offset;
  }
  #undef g2
...
/*! \brief Obtiene la fecha de inicio y caducidad del certificado.
 *
 * Obtiene la fecha de inicio y caducidad del certificado.
 *
 * \param cert
 *        El certificado en formato PEM.
 *
 ...
 *
 * \param notBefore
 *        [SALIDA] La fecha de inicio de validez del certificado.
 *
 * \param issuer
 *        [SALIDA] La fecha de fin de validez del certificado
 *
 * \retval 0
 *         Error
 *
 ...
 *         Ok
 */
int CRYPTO_CERT_Dates  ( unsigned char *cert, unsigned long tamCert, struct tm *notBefore, struct tm *notAfter )
{
  X509 *x509;
  ...
  if(notBefore)
    ASN1_UTCTIME2TM(X509_get_notBefore(x509),notBefore);
  if(notAfter)
    ASN1_UTCTIME2TM(X509_get_notAfter(x509),notAfter);
